"""Manual-first Codex adapter implementation."""
...
class CodexAdapter:
    """Manual-first Codex host adapter."""
    ...
    def discover(self, task: TaskSpec, run_dir: Path) -> str:
        """Build a local context pack for a manual Codex session."""
        ...
        return "\n".join(
            [
                f"# Codex Context Pack for {task.title}",
                "",
    ...
    def plan(self, task: TaskSpec, context: str) -> str:
        """Return a Codex-friendly planning brief."""
        prompt = self._build_plan_prompt(task, context)
        return "\n".join(
            [
                f"# Codex Plan for {task.title}",
                "",
                "This run is using the manual-first Codex adapter.",
                "Use the brief below in a Codex-capable session and preserve the aiwf artifact/state contract.",
                "",
                "## Suggested Codex Brief",
                "```text",
                prompt,
                "```",
            ]
    ...
    def execute(self, task: TaskSpec, plan: str, run_dir: Path) -> StageResult:
        """Write an implementation handoff brief and block for external Codex execution."""
        prompt = self._build_execute_prompt(task, plan, run_dir)
        prompt_path = run_dir / "codex-implement-prompt.md"
        prompt_path.write_text(prompt, encoding="utf-8")
        return StageResult(
            stage="implement",
            status=RunStatus.blocked,
            summary=f"Codex implementation handoff prompt written for {task.title}",
    ...
    def review(self, task: TaskSpec, run_dir: Path) -> dict[str, object]:
        """Write a review handoff brief and block for external Codex review."""
        prompt = self._build_review_prompt(task, run_dir)
        prompt_path = run_dir / "codex-review-prompt.md"
        prompt_path.write_text(prompt, encoding="utf-8")
        return {
            "summary": f"Codex review handoff prompt written for {task.title}",
            "issues": [],
            "mode": "manual",
    ...
    def _build_plan_prompt(self, task: TaskSpec, context: str) -> str:
        return "\n".join(
            [
                ...
                "Preserve the existing aiwf workflow, artifact, and resume semantics.",
                "Keep the implementation scoped to the task and avoid unrelated redesign.",
                "",
                "Task details:",
    ...
    def _build_review_prompt(self, task: TaskSpec, run_dir: Path) -> str:
        return "\n".join(
            [
                ...
                "Focus on correctness, missing validation, and follow-up work before the run is resumed.",
                "",
                "Inspect at minimum:",
