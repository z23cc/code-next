...
def test_run_doctor_reports_bridge_readiness_hints_from_tool_operations(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_probe(command_path: str) -> RpBridgeProbeResult:
        ...
        return RpBridgeProbeResult(
            ...
            tools=(
                ...
                RpToolInfo(name="file_search"),
                RpToolInfo(name="apply_edits"),
            ),
            error=None,
        )
    ...
    assert "repo-search=ready" in bridge_check.detail
    assert "destructive-editing=advertised" in bridge_check.detail
    assert "Destructive warning" in bridge_check.detail
