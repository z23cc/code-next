...
def test_rp_cli_bridge_list_tools_success_and_probe_available(tmp_path: Path) -> None:
    ...
    assert [tool.name for tool in result.tools] == ["file_search", "manage_selection"]
    assert result.tools[0].description == "Search files"
    assert probe.available is True
    assert [tool.name for tool in probe.tools] == ["file_search", "manage_selection"]
...
def _write_fake_rp_bridge_cli(tmp_path: Path, *, mode: str) -> Path:
    ...
    script_path.write_text(
        (
            ...
            "        raise SystemExit(9)\n"
            "    sys.stdout.write(json.dumps({'tools': [{'name': 'file_search', 'description': 'Search files'}, {'name': 'manage_selection'}]}))\n"
            "    sys.stdout.write('\\n')\n"
            "    raise SystemExit(0)\n"
            "\n"
