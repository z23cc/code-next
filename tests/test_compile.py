...
def test_compile_codex_writes_bundle_projection_and_manifest(tmp_path: Path) -> None:
    ai_root = _create_ai_sources(tmp_path)
    ...
    assert manifest["compiler"]["projection_contract"] == "codex-host-projection-v1"
...
def test_compiler_registry_exposes_host_specs_backed_by_adapter_contracts() -> None:
    ...
    assert claude_spec is COMPILER_SPECS["claude"]
    ...
    assert codex_spec is COMPILER_SPECS["codex"]
...
def test_compile_claude_projection_matches_compat_fixture(tmp_path: Path) -> None:
    ...
    assert _claude_projection_compat_view(projection) == expected
...
def test_compile_cross_host_projection_surface_matches_common_contract_structure(tmp_path: Path) -> None:
    ...
    assert _common_projection_contract_view(claude_projection) == _common_projection_contract_view(codex_projection)
...
def _claude_projection_compat_view(projection: dict[str, object]) -> dict[str, object]:
