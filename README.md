...
- `contracts lint` 契约检查
- `doctor` 工作区自检
- `compile claude` / `compile codex` 宿主投影与 drift manifest
...
- `--auto` 仅在宿主契约声明支持自动执行时生效；当前仅 `claude` 支持 auto。
- `rp` 与 `codex` 当前是 manual-first（通过 handoff prompt + `resume` 流程推进）。
...
.ai/runs/<run_id>/
  ...
  rp-agent-implement-prompt.md # manual-first RepoPrompt adapter
...
- `compile claude` 输出 `.claude/compiled/claude-bundle.md`、`claude-projection.json`、`manifest.json`
